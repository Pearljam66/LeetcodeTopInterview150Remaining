//
//  BinaryTreeMaximumPathSumTest.swift
//  RemainingLeetcode150Tests
//
//  Created by Sarah Clark on 12/26/24.
//

/*
 A path in a binary tree is a sequence of nodes where each pair of adjacent nodes in the sequence has an edge connecting them. A node can only appear in the sequence at most once. Note that the path does not need to pass through the root.
 The path sum of a path is the sum of the node's values in the path.
 Given the root of a binary tree, return the maximum path sum of any non-empty path.

 Example 1:
 Input: root = [1,2,3]
 Output: 6
 Explanation: The optimal path is 2 -> 1 -> 3 with a path sum of 2 + 1 + 3 = 6.

 Example 2:
 Input: root = [-10,9,20,null,null,15,7]
 Output: 42
 Explanation: The optimal path is 15 -> 20 -> 7 with a path sum of 15 + 20 + 7 = 42.

 Constraints:
 The number of nodes in the tree is in the range [1, 3 * 104].
 -1000 <= Node.val <= 1000
 */

import Testing

struct BinaryTreeMaximumPathSumTest {

    class TreeNode {
        var val: Int
        var left: TreeNode?
        var right: TreeNode?

        init() { self.val = 0; self.left = nil; self.right = nil; }
        init(_ val: Int) { self.val = val; self.left = nil; self.right = nil; }
        init(_ val: Int, _ left: TreeNode?, _ right: TreeNode?) {
            self.val = val
            self.left = left
            self.right = right
        }
    }

    class Solution {
        private var maxSum = Int.min

        func maxPathSum(_ root: TreeNode?) -> Int {
            maxPathSumHelper(root)
            return maxSum
        }

        private func maxPathSumHelper(_ node: TreeNode?) -> Int {
            guard let node = node else { return 0 }

            // Get the maximum path sum from left and right subtrees
            let leftSum = max(maxPathSumHelper(node.left), 0)
            let rightSum = max(maxPathSumHelper(node.right), 0)

            // Current path sum including the node and both subtrees
            let currentPathSum = node.val + leftSum + rightSum

            // Update maximum path sum if current path sum is greater
            maxSum = max(maxSum, currentPathSum)

            // Return maximum sum of path ending at current node
            return node.val + max(leftSum, rightSum)
        }
    }

    @Test func example1() {
        let root = TreeNode(1, TreeNode(2), TreeNode(3))
        #expect(Solution().maxPathSum(root) == 6)
    }

    @Test func example2() {
        let root = TreeNode(-10,
                             TreeNode(9),
                             TreeNode(20, TreeNode(15), TreeNode(7)))
        #expect(Solution().maxPathSum(root) == 42)
    }

}
